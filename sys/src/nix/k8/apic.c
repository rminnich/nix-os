...
/* Timer values are in fs.  To convert to apic tic values,
 * they must be multiplied by the number of tics/fs, a number
 * significantly less than one.
 * In apic->mul, we record 2⁴⁰·tics/fs.
 * To calculate the number of apic tics for a timeout,
 * we need to calculate (x * apic->mul) >> 40, but to avoid
 * overflow/underflow, we shift in two batches:
 */
#define fs2tics(x)	((long)((((x) >> 18) * apic->mul) >> 22))
