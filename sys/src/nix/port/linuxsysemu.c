...
linuxexit(Ar0*, va_list list)
{
	...
	if (up->linux & 128) print("%d:linuxexit %d\n", up->pid, val);
	up->linux = 0;
	pexit(exitstr, 1);
}
...
linuxsbrk(Ar0* ar0, va_list list)
{
	...
	*/
	...
	if (i < NSEG)
		ar0->p = ibrk(addr, i);
	if (up->linux & 128) print("%d:linuxsbrk for %p returns %p\n", up->pid, addr, ar0->p);
}
...
//	void linuxsbrk(Ar0* ar0, va_list list);
	...
	if (up->linux & 128) print("%d:linuxbrk va %#p: ", up->pid, (void *)va);
	//arg[0] = va;
	...
	if (va)	
		ar0->v = (void *)va;
	if (up->linux & 128) print("returns %#p\n", va);
...
linuxwritev(Ar0 *ar0, va_list list)
{
	...
	if (up->linux & 128) print("%d:linuxwritev (%d, %p, %d):", up->pid, fd, iov, iovcnt);
	validaddr(iov, iovcnt * sizeof(*iov), 0);
	/* don't validate all the pointers in the iov; sys_write does this */	
	for(i = 0; i < iovcnt; i++) {
		Ar0 war0;
		uintptr arg[3];
		if (up->linux & 128) print("[%p,%d],", iov[i].base, iov[i].len);
		arg[0] = fd;
...
linuxsocketcall(Ar0 *ar0, va_list list)
{
	...
	if (up->linux & 128) print("%d:linuxsocketcall (%d, %p):", up->pid, fd, args);
...
void linuxmmap(Ar0 *ar0, va_list list)
{
	...
	if (up->linux & 128) print("%d:CNK: mmap %p %#x %#x %#x %d %#ulx\n", up->pid, v, length, prot, flags, fd, offset);
	if (fd == -1){
		unsigned char *newv, *oldv;
		uintptr args[1];
		args[0] = 0;
		linuxsbrk(ar0, (va_list) args);
		if (up->linux & 128) print("%d:mmap anon: current is %p\n", up->pid, ar0->v);
		oldv =ar0->v;
		newv =  ((unsigned char *)oldv) + length;
		if (up->linux & 128) print("%d:mmap anon: ask for %p\n", up->pid, newv);
		args[0] = (uintptr) newv;
		linuxsbrk(ar0, (va_list) args);
		if (up->linux & 128) print("%d:mmap anon: new is %p\n", up->pid, ar0->v);
...
void sigaction(Ar0 *ar0, va_list list)
{
	...
	if (up->linux & 128) print("%d:sigaction, %p %p\n", up->pid, act, oact);
	ar0->i = 0;
}
...
void rt_sigprocmask(Ar0 *ar0, va_list list)
{
	...
	if (up->linux & 128) print("%d:sigaction, %d %p %p %d\n", up->pid, how, set, oset, size);
	ar0->l = 0;
}
...
void futex(Ar0 *ar0, va_list list)
{
	...
	USED(uaddr2);
	USED(val3);
	if (up->linux & 128) print("%d:futex, uaddr %p op %x val %x uaddr2 %p timeout  %p val3 %x\n", up->pid, 
	...
	switch(op) {
		...
		case FUTEX_WAIT: 
			...
			if (up->linux & 128) print("%d:futex: value at %p is %#x, val is %#x\n", up->pid, uaddr, *uaddr, val);
			if (*uaddr != val) {
				ar0->i = -11;
				return;
			}
			...
			if (up->linux & 128) print("%d:Not going to sleep\n", up->pid);
			break;
	}
}
...
void linuxmprotect(Ar0 *ar0, va_list list)
{
	...
	if (up->linux & 128) print("%d:mprotect: %#x %#x %#x\n", up->pid, addr, len, prot);
	ar0->i = 0;
}
...
linuxdupseg(Segment **seg, int segno, int share)
{
	...
	case SG_STACK:
		...
		if(share){ if (up->linux & 128) print("CLONE STACK IS SHARE\n");
			goto sameseg;
		}
		/* that is all the change */
if (up->linux & 128) print("CLONE STACK IS NEW\n");
	...
	case SG_BSS:		/* Just copy on write */
		if(share)
			goto sameseg;
if (up->linux & 128) print("CLONE NEW BSS\n");
		n = newseg(s->type, s->base, s->size);
		break;
...
void linuxclone(Ar0 *ar0, va_list list)
{
	...
	if (up->linux & 128) print("%d:CLONE: %#x %#x\n", up->pid, flags, stack);
	if (flags != 0x7d0f00) {
...
void getrusage(Ar0 *ar0, va_list list)
{
	...
	if (up->linux & 128) print("%d:getrusage: %s %p:",up->pid,  !what? "self" : "kids", r);
	memset(r, 0, sizeof(*r));
	if (what) {
		...
		if (up->linux & 128) print("%#lx:%#lx, ", up->time[0], up->time[1]);
	}
	if (up->linux & 128) print("%#x:%#x\n", r->ru_utime.tv_sec, r->ru_stime.tv_sec);
